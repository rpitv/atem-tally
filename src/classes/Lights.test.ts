      ...
      for (let j = 0; j < diode.pins.length; j++) {
        ...
        if ((spy.mock.instances[spyIndex + i] as any).pin === diode.pins[j]) {
          ...
          if (diode instanceof DigitalDiode) {
            ...
            expect((spy.mock.calls[spyIndex + i] as any)[0]).toEqual(expected);
...
it("Does not allow pin IDs less than 0.", () => {
  expect(() => {
    testBasicWrites([new PwmDiode(10, -100, 1353, 50, 150, 10, false, false)]);
  }).toThrow(Error);
});
...
it("Does not allow PWM writes greater than 255.", () => {
  expect(() => {
    testBasicWrites([new PwmDiode(10, 100, 1353, 50, 256, 10, false, false)]);
  }).toThrow(Error);
});
...
it("Does not allow digital writes to all three LEDs at the same time.", () => {
  expect(() => {
    testBasicWrites([new DigitalDiode(1, 2, 3, true, true, true, false, true)]);
  }).toThrow(Error);
  expect(() => {
    testBasicWrites([
